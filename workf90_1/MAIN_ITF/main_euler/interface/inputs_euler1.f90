...
read(1,'(4f20.12)') (((uu(i,j)%value(k),k=1,4),i=nxll-3,nxll+nx+2),j=nyll-3,nyll+ny+2)
read(1,'(4f20.12)') ((uu(i,j)%gamma,i=nxll-3,nxll+nx+2),j=nyll-3,nyll+ny+2)
close(1)
! Input the solution in smooth region.

open(5,file='d:\workf90_1\input\initgd.dat')
read(5,'(9a8)') &
((ggd_cell(i,j)%region, i=nxll-3,nxll+nx+2),j=nyll-3,nyll+ny+2)
read(5,'(15i5)') &
 ...
 if(cvv(i)%status.eq.'awake'.or.cvv(i)%status.eq.'yawn  ') then 
  ...
  if(cvv(i)%status.eq.'awake ') then
   ...
   do while(address%idx.gt.0) 
	...
	read(2,'(f20.12)') p_cell%l_state%gamma
	read(2,'(4f18.12)') (p_cell%r_state%value(k),k=1,4)
    read(2,'(f20.12)') p_cell%r_state%gamma
	read(2,'(4f18.12)') (p_cell%or_state%value(k),k=1,4)
    read(2,'(f20.12)') p_cell%or_state%gamma
...
read(7,'(2a12)') boundary_type_1, boundary_type_2
if(boundary_type_1.eq.'periodic    ') read(7,'(f18.12)') moved_time
close(7)
