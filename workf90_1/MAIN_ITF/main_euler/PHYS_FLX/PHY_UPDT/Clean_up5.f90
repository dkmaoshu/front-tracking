...
real(8) :: l_x_velo, r_x_velo, or_x_velo, l_y_velo, r_y_velo, or_y_velo, l_pressure, &
           r_pressure, or_pressure, or_density, ttt
real(8), dimension(2) :: normal 
real(8) :: l_area, r_area, alpha, beta
...
do while(associated(temp).and.head_switch) 
 ...
 if(temp%l_stk.ne.adss_info(0,0).or.temp%r_stk.ne.adss_info(0,0)) then
  ...
  if(temp%l_stk.ne.adss_info(0,0)) then
   ...
   r_area=side_area(temp%g_cell,'right ')
   r_x_velo=x_velocity(temp%p_cell%r_state)
   r_y_velo=y_velocity(temp%p_cell%r_state)
   r_pressure=pressure(temp%p_cell%r_state)
    
   call visit(temp%l_stk,temp_m)
   l_x_velo=x_velocity(temp_m%p_cell%l_state)
   l_y_velo=y_velocity(temp_m%p_cell%l_state)
   ...
   l_area=side_area(temp%g_cell,'left  ')
   l_x_velo=x_velocity(temp%p_cell%l_state)
   l_y_velo=y_velocity(temp%p_cell%l_state)
   l_pressure=pressure(temp%p_cell%l_state)
    
   call visit(temp%r_stk,temp_m)
   r_x_velo=x_velocity(temp_m%p_cell%r_state)
   r_y_velo=y_velocity(temp_m%p_cell%r_state)
  ...
  alpha=l_area+1.0d-6; beta=r_area+1.0d-6
...
real(8) :: this_area, that_area, alpha, beta
 ...
 select case(p_cell%wv_nb)
  ...
  case(2)
   ...
   if(temp%l_stk.ne.adss_info(0,0).or.temp%r_stk.ne.adss_info(0,0)) then
    ...
    alpha=this_area+1.0d-6; beta=that_area+1.0d-6
    p_cell%or_state=p_cell%or_state-(beta*difference)/(alpha+beta)
