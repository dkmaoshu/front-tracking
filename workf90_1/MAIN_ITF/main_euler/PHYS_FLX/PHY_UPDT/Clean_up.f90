...
private g_cell, p_cell, l_store, r_store, cleaning, return_data, &
...
type(critical_cell), pointer :: temp_s
type(state) :: ss_phy_state
real(8) :: ss_area, ss_pressure
...
! And then decompose the conservation difference according to the characteristics.
   ...
   select case(p_cell%wv_nb)
    ...
    case(1)
     ...
     call riemann(l_state,r_state,normal,1,x_posi,y_posi,c1,c2, &
	              velocity,wave_type)
...
!      beta=dsign(1.0d0,beta)*dmin1(dabs(beta),dabs(beta)**(3.0d0/2.0d0))
    ...
    case(3) 
     ...
     call riemann(l_state,r_state,normal,3,x_posi,y_posi,c1,c2, &
                  velocity,wave_type)
     call transform_1(c1,normal,nc1)
     call transform_1(c2,normal,nc2)
     vec(4)=nc2-nc1
...
!  important changes for two different gases 
   ...
   if(temp%l_stk.eq.adss_info(0,0).and.temp%r_stk.eq.adss_info(0,0)) then
    ...
    if(temp%l_stk.ne.adss_info(0,0)) then
     ...
     if(temp_s%l_stk.eq.temp%address) then
      ss_area=side_area(temp_s%g_cell,'right ')
      call find_physical_state(temp_s%p_cell%r_state,(/1.0d0,0.0d0/),ss_phy_state)
      ss_pressure=ss_phy_state%value(4)
     else
      if(temp_s%r_stk.eq.temp%address) then
       ss_area=side_area(temp_s%g_cell,'left  ')
       call find_physical_state(temp_s%p_cell%l_state,(/1.0d0,0.0d0/),ss_phy_state)
       ss_pressure=ss_phy_state%value(4)
      else
       call error_message
      end if
     end if
     or_pressure=(l_area-ss_area)*l_pressure+ss_area*ss_pressure+r_area*r_pressure
    else
     if(temp%r_stk.ne.adss_info(0,0)) then
      ...
      or_pressure=l_area*l_pressure+ss_area*ss_pressure+(r_area-ss_area)*r_pressure
     else
      call error_message
     end if
    end if
