...
t2 = 0.5d0 * b1
t3 = b1 * vym
evll(1,1) = 0.5d0 * ( b2 + t0 )
evll(1,2) = -0.5d0 * ( t1 + rcm )
evll(1,3) = -0.5d0 * t3
evll(1,4) = t2
evll(2,1) = - vym
evll(2,2) = 0.0d0
...
evll(3,3) = t3
evll(3,4) = -b1
evll(4,1) =  0.5d0 * ( b2 - t0 )
evll(4,2) = -0.5d0 * ( t1 - rcm )
evll(4,3) = -0.5d0 *   t3
evll(4,4) = t2

! Call the WENO slover.

call weno(evl,evr,evll,evrr,f,wuu,fff,wu,2,flux)
...
evll(4,1) =  0.5d0 * ( b2 - t0 )
evll(4,2) = -0.5d0 *   t3
evll(4,3) = -0.5d0 * ( t1 - rcm )
evll(4,4) = t2

! Call the WENO solver.

call weno(evl,evr,evll,evrr,f,wuu,fff,wu,3,flux)
...
subroutine weno(evl,evr,evll,evrr,f,wuu,fff,wu,xy_dir,flux)
...
real(8), dimension(4), intent(out) :: flux
real(8), dimension(4,4), intent(in) :: evl, evr, evll, evrr
real(8), dimension(-2:3,4), intent(in) :: f, wuu, fff
