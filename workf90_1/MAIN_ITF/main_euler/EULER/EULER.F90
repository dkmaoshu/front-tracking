...
do i=1, 4
 if(dabs(main_jump_normal_tangential(i)).lt.0.3d0) then
  left_reliability(i)=0.0d0; right_reliability(i)=0.0d0
 else
...
! Third, compute the discontinuity position in each component.
position=error_data
do i=1,4
 if(dabs(main_jump_normal_tangential(i)).ge.0.3d0) then
  position(i)=ordinary_normal_tangential(i)/main_jump_normal_tangential(i)
 end if
end do
...
select case(wave_number)
 case(1,3)
  reliability=dmin1(left_reliability(1),right_reliability(1))
  final_position=reliability*position(1)
  sum=reliability
  reliability=dmin1(left_reliability(2),right_reliability(2))
  final_position=final_position+reliability*position(2)
  sum=sum+reliability
  reliability=dmin1(left_reliability(4),right_reliability(4))
  final_position=final_position+reliability*position(4)
  sum=sum+reliability
 ...
 case(2)
  ...
  final_position=reliability*position(1)
  sum=reliability
  reliability=dmin1(left_reliability(3),right_reliability(3))
  if(reliability.gt.0.5d0) then
   final_position=final_position+reliability*position(3)
   sum=sum+reliability
   final_position=final_position/sum
  else
   delta=dabs(main_jump_normal_tangential(3))*2.0d0*h
   if(main_jump_normal_tangential(3).gt.0.0d0) then
    if(ordinary_normal_tangential(3).gt.0.5d0*main_jump_normal_tangential(3)) then
	 final_position=final_position+delta
    end if
	if(ordinary_normal_tangential(3).lt.-0.5d0*main_jump_normal_tangential(3)) then
	 final_position=final_position-delta
    end if
   end if
   if(main_jump_normal_tangential(3).lt.0.0d0) then
    if(ordinary_normal_tangential(3).lt.0.5d0*main_jump_normal_tangential(3)) then
     final_position=final_position+delta
    end if
    if(ordinary_normal_tangential(3).gt.-0.5d0*main_jump_normal_tangential(3)) then
...
function courant_number(a,x,y) result(c)
...
select case(num)
 ...
 case(2); c1%value=wml%value; c2%value=wmr%value
