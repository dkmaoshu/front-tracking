...
open(4,file='d:\workf90_1\accuracy_check\accuracy_check3\input\initt.dat') 
read(4,'(e25.15)') current_time
read(4,'(i5)') total_step
close(4)
! Input the initial time.

open(1,file='d:\workf90_1\accuracy_check\accuracy_check3\input\initg.dat')
read(1,'(2e25.15)') x_width, y_width
...
open(1,file='d:\workf90_1\accuracy_check\accuracy_check3\input\initu.dat')
read(1,'(4e25.15)') &
(((uu(i,j)%value(k),k=1,4),i=nxll,nxll+nx-1),j=nyll,nyll+ny-1)
close(1)
! Input the solution in smooth region.

open(5,file='d:\workf90_1\accuracy_check\accuracy_check3\input\initgd.dat')
read(5,'(9a8)') &
...
open(2,file='d:\workf90_1\accuracy_check\accuracy_check3\input\initd.dat')
 ...
 if(cvv(i)%status.eq.'awake'.or.cvv(i)%status.eq.'yawn  ') then 
  ...
  if(cvv(i)%status.eq.'awake ') then
   ...
   do while(address%idx.gt.0) 
    ...
    read(2,'(2e25.15)') g_cell%dis_posi(1), g_cell%dis_posi(2)
    read(2,'(2i5)') g_cell%edge(1), g_cell%edge(2)
    read(2,'(4a8)') (g_cell%point(jj),jj=1,4)
    
    read(2,'(4e25.15)') (p_cell%l_state%value(k),k=1,4)
	read(2,'(4e25.15)') (p_cell%r_state%value(k),k=1,4)
	read(2,'(4e25.15)') (p_cell%or_state%value(k),k=1,4)
...
open(6,file='d:\workf90_1\accuracy_check\accuracy_check3\input\initnd.dat')
do i=1,nodes_number
 read(6,'(a8)') ndd(i)%status
 if(ndd(i)%status.eq.'awake') then 
!  read(6,'(a8)') ndd(i)%n_type
  read(6,'(i5,i5)') n_cell%x_idx, n_cell%y_idx
  read(6,'(4e25.15)') (n_cell%or_state%value(k),k=1,4)
  ndd(i)%n_cell=n_cell
...
open(7,file='d:\workf90_1\accuracy_check\accuracy_check3\input\boundary_situation.dat')
...
end subroutine input_and_setup_f


end module input_setup_f