...
! type(auxi_crit_cell), pointer :: temp_pos, temp_neg 
! real(8) :: xxx, yyy, zzz, aaa
! real(8), dimension(2) :: pt
! integer :: ii
! xxx=0.0d0; yyy=0.0d0 !; nullify(temp_pos); nullify(temp_neg)
...
!  if(gc_c%x_idx.eq.40.and.gc_c%y_idx.eq.-1) then
!   print*, pc_c%or_state%value(1), pc_c%or_state%value(2)
!   print*, pc_c%or_state%value(3), pc_c%or_state%value(4)
!  end if
...
!! Compute neck width
!   
!  if(gc_c%g_type.eq.'xx '.and.dabs(gc_c%dis_posi(1)-gc_c%dis_posi(2)).lt.0.05d0) then
!   aaa=dfloat(gc_c%x_idx)*h+0.5d0*(gc_c%dis_posi(1)+gc_c%dis_posi(2))*h-1.0d0
!   if(aaa.lt.xxx.and.aaa.gt.0.0d0) then
!    xxx=aaa; print*, '+   ', gc_c%x_idx, gc_c%y_idx
!   end if
!   if(aaa.gt.yyy.and.aaa.lt.0.0d0)  then
...
!   
!  if(gc_c%g_type.eq.'xx '.and.dfloat(gc_c%y_idx)*h.gt.0.365d0) then
!   aaa=dfloat(gc_c%x_idx)*h+0.5d0*(gc_c%dis_posi(1)+gc_c%dis_posi(2))*h-1.0d0
!   if(aaa.gt.xxx) then
!    xxx=aaa; print*,'+ ', gc_c%x_idx, gc_c%y_idx
!   end if
...
!  end if
!   
!  if(gc_c%g_type.eq.'yy '.and.gc_c%x_idx.eq.160) then
!   zzz=dfloat(gc_c%y_idx)*h+0.5d0*(gc_c%dis_posi(1)+gc_c%dis_posi(2))*h
!  end if
!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!! Compute neck width for Air-He.
!   
!  if(gc_c%g_type.eq.'yy '.and.dabs(gc_c%dis_posi(1)-gc_c%dis_posi(2)).lt.0.1d0) then
!   aaa=dfloat(gc_c%y_idx)*h+0.5d0*(gc_c%dis_posi(1)+gc_c%dis_posi(2))*h
!   if(aaa.lt.xxx.and.aaa.gt.0.0d0) then
!    xxx=aaa; print*,'+ ', gc_c%x_idx, gc_c%y_idx
!   end if
!   if(aaa.gt.yyy.and.aaa.lt.0.0d0)  then
...
!! Compute streamwise width.
!
! xxx=dfloat(temp_pos%g_cell%y_idx)*h; yyy=dfloat(temp_neg%g_cell%y_idx)*h
! if(acvv(cv_nmb)%begin%next%g_cell%x_idx.eq.0) then
!  do while(associated(temp_pos))
!   temp_pos=>temp_pos%previous
!   if(temp_pos%g_cell%x_idx.eq.160) exit
!   do ii=1, 2
!    call pick_point(temp_pos%g_cell,ii,pt)
!    aaa=(dfloat(temp_pos%g_cell%y_idx)+pt(2))*h
!    if(aaa.lt.xxx) then
!     xxx=aaa
!    end if
!   end do
!  end do
!  do while(associated(temp_neg))
!   temp_neg=>temp_neg%next
!   if(temp_neg%g_cell%x_idx.eq.160) exit
!   do ii=1, 2
!    call pick_point(temp_neg%g_cell,ii,pt)
!    aaa=(dfloat(temp_neg%g_cell%y_idx)+pt(2))*h
!    if(aaa.lt.yyy) then
!     yyy=aaa
!    end if
!   end do
...
if(.not.associated(temp_g)) then
 print*, 'No match in the visit.'; call error_message
end if
...
length=error_data; l_area=error_data; r_area=error_data
