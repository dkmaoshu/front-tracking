...
integer :: i_present, j_present, i_stacked, j_stacked
 ...
 if(associated(temp_stack)) then
  ...
  do j=1,2
   ...
   if(g_stack%edge(j).eq.g_cell%edge(i)) then
    select case(g_cell%edge(i))
     case(1,3)
      if(i_stacked.eq.error_index.or.i_present.eq.error_index) then
     ...
     case(2,4)
      if(j_stacked.eq.error_index.or.j_present.eq.error_index) then
       call error_message; pause
      end if
