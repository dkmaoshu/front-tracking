...
select case(g_cell%g_type)
 case('xx ')
  call smth_dpr(i,j,temp,ul,'xx ','left  ','critical',.true.,3)
  call smth_dpr(i,j,temp,ur,'xx ','right  ','critical',.true.,3)
 case('yy ')
  call smth_dpr(i,j,temp,ul,'yy ','left  ','critical',.true.,3)
  call smth_dpr(i,j,temp,ur,'yy ','right  ','critical',.true.,3)
 case default; call error_message
end select
...
! Compute the left and right differences.
 ...
 select case(g_neigh%g_type)
  case('xx ')
   call smth_dpr(ii,jj,temp,ul,'xx ','left  ','critical',.true.,3)
   call smth_dpr(ii,jj,temp,ur,'xx ','right  ','critical',.true.,3)
  case('yy ')
   call smth_dpr(ii,jj,temp,ul,'yy ','left  ','critical',.true.,3)
   call smth_dpr(ii,jj,temp,ur,'yy ','right  ','critical',.true.,3)
  case default; call error_message
 end select
 if(g_cell%point(1).eq.'left  ') then
  left_difference=ul(0)-ul(-1); right_difference=ur(1)-ur(0)
...
subroutine treat_garlic_heads(acv)
! Smash the garlic-head-like structure by recovering discontinuity
! positions in $xy$-type discontinuity cells.
...
do while(associated(temp).and.head_switch) 
 ...
 if(g_cell%g_type.eq.'xy ') then
  call if_fat(g_cell,p_cell,faty)
  if(faty) then
   do i=1,2
    select case(i)
     case(1)
      if(temp%previous%partner.ne.'single') cycle
      g_celln=temp%previous%g_cell
     case(2)
      if(temp%next%partner.ne.'single') cycle 
      g_celln=temp%next%g_cell
    end select	 
	call treat_garlic(g_cell,g_celln,i)
   end do
  end if   	 
...
end subroutine if_fat


subroutine treat_garlic(g_cell,g_celln,i)
...
call find_single(g_cell,single)
if(g_celln%g_type.eq.'xy ') return
...
select case(single)
 ...
 case(1)
  ...
  select case(g_cell%edge(i))
   case(1)
    if(g_celln%g_type.eq.'xx ') then
     if(g_celln%mdis_posi(1).gt.0.5d0) then
      g_cell%mdis_posi(i)=g_celln%mdis_posi(1)
     end if
    else
     call error_message
    end if
   case(4) 
    if(g_celln%g_type.eq.'yy ') then
     if(g_celln%mdis_posi(2).gt.0.5d0) then
      g_cell%mdis_posi(i)=g_celln%mdis_posi(2)
     end if
    else
     call error_message
    end if
 ...
 case(2)
  ...
  select case(g_cell%edge(i))
   case(1)
    if(g_celln%g_type.eq.'xx ') then
     if(g_celln%mdis_posi(1).lt.-0.5d0) then
      g_cell%mdis_posi(i)=g_celln%mdis_posi(1)
     end if
    else
     call error_message
 ...
 case(3)
  ...
  select case(g_cell%edge(i))
   ...
   case(2) 
    if(g_celln%g_type.eq.'yy ') then
     if(g_celln%mdis_posi(2).lt.-0.5d0) then
      g_cell%mdis_posi(i)=g_celln%mdis_posi(2)
     end if
    else
     call error_message
...
end subroutine treat_garlic


end subroutine treat_garlic_heads
