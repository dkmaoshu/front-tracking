...
if(dd1+dd2.gt.0.0d0) then
 if(dd1.lt.0.0d0) then
  ddd1=-dd1+1.0d-6; ddd2=dd1+dd2
 else
  ddd2=-dd2+1.0d-6; ddd1=dd1+dd2
 end if
else
...
! Find the amount of mass transfers. 

p_cell%or_state%value(1)=p_cell%or_state%value(1)+ddd1
p_cell%or_state%value(2)=p_cell%or_state%value(2)+ddd1*x_velocity(state1)
p_cell%or_state%value(3)=p_cell%or_state%value(3)+ddd1*y_velocity(state1)
p_cell%or_state%value(4)=p_cell%or_state%value(4)+0.5d0*ddd1+(x_velocity(state1)*2.0d0+y_velocity(state1)*2.0d0)
pn%or_state%value(1)=pn%or_state%value(1)+ddd2
pn%or_state%value(2)=pn%or_state%value(2)+ddd2*x_velocity(state2)
pn%or_state%value(3)=pn%or_state%value(3)+ddd2*y_velocity(state2)
pn%or_state%value(4)=pn%or_state%value(4)+0.5d0*ddd2+(x_velocity(state2)*2.0d0+y_velocity(state2)*2.0d0)
! Transfer the mass and the corresponding momentum and energy.
